...
| **P-S2** | E2E tests against a LEZ sequencer (**standalone**) in CI | ✅ | Green twice. The later run [34302452494](https://github.com/pramadanif/lp-0002-private-multisig/actions/runs/34302452494) on pin `a8755b4` finished **all eleven jobs green**, its e2e job in 3 h 40 m, two proofs of **96 minutes each**, ending `VERIFIED from public chain data alone`; nothing behaviour-bearing has changed since that pin — `git diff a8755b4..HEAD -- crates programs artifacts Cargo.lock rust-toolchain.toml scripts/e2e-local-sequencer.sh demo.sh` is empty. Before it, run [34275830322](https://github.com/pramadanif/lp-0002-private-multisig/actions/runs/34275830322) on pin `c530fc8`, job "e2e against a real standalone LEZ sequencer": **success, 2 h 54 m**. Not a skipped pass — the job asserts `RISC0_DEV_MODE=0`, its log carries `sequencer is live — getLastBlockId = 1`, **two proofs of 75 minutes each**, `executed and confirmed`, and the INV-7 check reporting the payee holding 60 of the 60 approved with 40 left in the treasury. The same script also passes on a laptop, in 21–22 min per proof ([evidence](../evidence/e2e-local-2026-09-09.md)); an earlier CI run, 34052567273, passed the same way on 2026-09-06 |
...
| **P-S6** | Narrated video showing terminal output incl. proof generation, confirming `RISC0_DEV_MODE=0` | ✅ | [https://youtu.be/7gRweVxWEL4](https://youtu.be/7gRweVxWEL4) — narrated by the builder, not synthesised. `./demo.sh` starts at [t=100](https://youtu.be/7gRweVxWEL4?t=100) with the `RISC0_DEV_MODE=0` banner against a real sequencer; the lifecycle completes at [t=194](https://youtu.be/7gRweVxWEL4?t=194); the Basecamp module reads the deployed multisig from chain at [t=246](https://youtu.be/7gRweVxWEL4?t=246). The commit on camera is `138c683` — `demo.sh` and `verify-onchain.sh` both print it |
...
**The cause that closed two submissions outright — no narrated video (#87, #102) — is closed here
too**, and narrated by the builder rather than synthesised, which is what closed a third.
...
entitlement its own QML sandbox uses — the host's bug, reproduced and filed, not a gap in this
module.
