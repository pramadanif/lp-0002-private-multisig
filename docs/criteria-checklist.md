...
| **P-U2** | Basecamp GUI: local build, downloadable assets, loadable | ◐ | `app/private_multisig.lgx`, 2,632,590 bytes, sha256 `c8056283…a7f9c7b5`. Built by `./scripts/build-basecamp.sh`, which checks every one of the thirteen `extern "C"` symbols the UI calls before packaging. **It installs in Logos Basecamp 0.2.3 and opens from Applications → Blockchain, and its panels render — but inside Basecamp they are inert.** Basecamp reaches a module's C++ through a *replica factory* plugin loaded beside it (`LogosQmlBridge: no replica factory plugin registered for "private_multisig"`); without one, `logos.module()` returns null and the module says so in its own footer. A first factory, reconstructed from the host binary, made Basecamp crash on startup and was withdrawn — see [tried-failed.md](tried-failed.md). **What *is* proven** is the layer beneath: `./scripts/check-basecamp-contract.sh` drives the plugin's own slots against the live testnet and decodes the deployed 2-of-3, its proposal and the wallet accounts — but it does that in a harness, not through Basecamp's bridge. **Gaps:** the factory, and the package carries only the `darwin-arm64` variant |
...
| **P-S2** | E2E tests against a LEZ sequencer (**standalone**) in CI | ✅ | Run [34275830322](https://github.com/pramadanif/lp0002/actions/runs/34275830322) on pin `c530fc8`, job "e2e against a real standalone LEZ sequencer": **success, 2 h 54 m**. Not a skipped pass — the job asserts `RISC0_DEV_MODE=0`, its log carries `sequencer is live — getLastBlockId = 1`, **two proofs of 75 minutes each**, `executed and confirmed`, and the INV-7 check reporting the payee holding 60 of the 60 approved with 40 left in the treasury. The same script also passes on a laptop, in 21–22 min per proof ([evidence](../evidence/e2e-local-2026-09-09.md)); an earlier CI run, 34052567273, passed the same way on 2026-09-06 |
...
**What is outstanding:** P-S6, the narrated video, which is not recorded. And P-U2: the module
installs and opens, and the code behind it is proven against the live chain, but inside Basecamp the
panels are inert for want of a replica factory plugin — the piece that lets `logos.module()` hand the
QML the backend. An earlier claim here said the module "reads the deployed multisig" in Basecamp.
That was wrong: the reads were through a harness of our own, whose stub `logos` returned the object
directly and so modelled a host that needs no factory.
