...
| Gate | State | Proof |
|------|-------|-------|
| **L1** Basecamp package | **LOCKED** | `.lgx` 2631589 bytes, sha256 `5d6f70f3a34b754f83440638b906a78bffca7d9257bbcbbe2c30bc210fad98a0`. Installs in Basecamp 0.2.3 and opens from Applications → Blockchain (host log: `ui-host: loaded plugin "private_multisig"`, `Successfully loaded UI module`). Wiring proven, not assumed: `./scripts/check-basecamp-contract.sh` asserts the plugin publishes the API Basecamp replicates and that `Main.qml` resolves it via `logos.module()`; with `PMSIG_CONTRACT_LIVE=1` it fetched the deployed config through the plugin's own slots and decoded the same 2-of-3. Reverting the QML to the old context-property form fails the check with 47 unresolved bindings. Scope: `darwin-arm64` only |
