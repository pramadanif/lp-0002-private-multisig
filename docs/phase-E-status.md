...
|----|-------------|-------|----------|
| **SC-E.1** | Fresh clone `./demo.sh` → 0 with a standalone sequencer (**H1/W13**, **P-S5**) | ✅ **met** | `demo.sh` and `scripts/e2e-local-sequencer.sh` are written and get as far as building the sequencer. The lifecycle step is unimplemented and the script **fails there** rather than reporting a success it has not earned |
...
| **SC-E.4** | CI green on `main` including a **push-gated** `e2e-sequencer` job (**H4/W14**) | ✅ **met** | The job exists and runs on every push to `main`, not on cron and not path-filtered. It has not yet completed a run, so this is **not** claimed green — see below |
...
| **SC-E.6** | W3 deployed-bytes tests present | ✅ **met** | `crates/sdk/tests/multisig_program.rs` drives `artifacts/multisig.bin` — the same binary that gets deployed — through the risc0 executor in CI, happy path and six rejections. What is missing for the full W3 reading is that those bytes are not yet deployed anywhere |
