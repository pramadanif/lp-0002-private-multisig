...
|--------|-----------|------|
| CI runs real LEZ sequencer e2e, green | ✅ **green, twice** | Run [34302452494](https://github.com/pramadanif/lp-0002-private-multisig/actions/runs/34302452494) on pin `a8755b4`, all eleven jobs green, e2e job 3 h 40 m with two 96-minute proofs. Before it, run [34275830322](https://github.com/pramadanif/lp-0002-private-multisig/actions/runs/34275830322) on pin `c530fc8`, 2 h 54 m: a real standalone sequencer, two proofs of **75 minutes each** with `RISC0_DEV_MODE=0`, execute at full M, verified from chain data. `e2e-sequencer` runs on every push to `main` — not on cron, not path-filtered, which is the shape #125 was pulled up for. An earlier run passed the same way on 2026-09-06 |
| Testnet evidence, live explorer links | ◐ **deployed and verified; the index lags** | Full lifecycle live on the public testnet and re-read from public data on 2026-09-09 (`verify-onchain.sh`, exit 0) — [DEPLOYMENT.md](DEPLOYMENT.md). The explorer renders the two program deployments but has not yet indexed the five lifecycle transactions; `check-explorer-links.sh` names them and exits 75 rather than passing quietly. The earlier deployment at blocks 43002–43065 was lost to a testnet reset and is not cited anywhere |
| CU cost | ✅ | On-chain CU for all four instructions, plus client proving: 602,662 cycles, 115.97 s standalone, and a composed approval at ≈19 min 26 s / 8.74 GB — repeated twice more on 2026-09-09 at 22 and 21 min. [cu-costs.md](cu-costs.md) |
| `demo.sh` genuinely DEV_MODE=0 | ✅ | Set at the entrypoint, never in a child. `check-dev-mode-clobber.sh` enforces exactly the #97 failure and is mutation-tested. The script has now completed unattended twice on the same day — once in CI, once on a laptop ([evidence](../evidence/e2e-local-2026-09-09.md)) |
| Basecamp module, working on camera | ◐ | It installs, opens, and reads the deployed multisig through its own plugin slots — `check-basecamp-contract.sh`, which CI runs. What is left is filming it |
