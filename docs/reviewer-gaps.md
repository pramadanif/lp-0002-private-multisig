...
|--------|-----------|------|
| CI runs real LEZ sequencer e2e, green | ✅ **green** | Run [34275830322](https://github.com/pramadanif/lp-0002-private-multisig/actions/runs/34275830322) on pin `c530fc8`, 2 h 54 m: a real standalone sequencer, two proofs of **75 minutes each** with `RISC0_DEV_MODE=0`, execute at full M, verified from chain data. `e2e-sequencer` runs on every push to `main` — not on cron, not path-filtered, which is the shape #125 was pulled up for. An earlier run passed the same way on 2026-09-06 |
